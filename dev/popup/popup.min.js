...
function toggleInjection(e) {
    ...
    chrome.storage.sync.set({ injectionEnabled: enabled }, () => {
        // Forzar actualización de todos los scripts
    ...
    e.type = "file", e.accept = ".js,.user.js", e.onchange = e => {
        ...
        if (e) {
            ...
            t.onload = e => {
                ...
                console.log("Importing script content:", scriptContent.substring(0, 100) + "...");
                ...
                chrome.storage.local.set({
                    ...
                    if (chrome.runtime.lastError) {
                        ...
                        alert("Error importing script: " + chrome.runtime.lastError.message);
                        return;
                    }
                    console.log("Script content stored successfully, opening editor");
                    chrome.tabs.create({
...
function installDetectedScript() {
    ...
    if (t) {
        console.log("Storing script content for editor:", t.substring(0, 100) + "...");
        chrome.storage.local.set({
            tempScriptContent: t
        }, function() {
            if (chrome.runtime.lastError) {
                console.error("Error storing script content:", chrome.runtime.lastError);
                alert("Error storing script: " + chrome.runtime.lastError.message);
                return;
            }
            console.log("Script content stored successfully for detected script");
            chrome.tabs.create({
                url: chrome.runtime.getURL("editor/editor.html?loadTemp=true")
            });
        });
        ...
        alert("No script content found to install");
    }
}
...
function loadAllScripts() {
    ...
    chrome.runtime.sendMessage({
        ...
        if (e && e.scripts) {
            0 === e.scripts.length ? t.innerHTML = '<div class="empty-state">No scripts installed</div>' : (t.innerHTML = "", e.scripts.forEach(e => {
                t.appendChild(createScriptElement(e, !0))
            }));
        } else if (e && e.error) {
            console.error("Error loading scripts:", e.error);
            t.innerHTML = `<div class="empty-state">Error loading scripts: ${e.error}</div>`;
        } else {
            console.error("Error loading scripts: No scripts or unexpected response.", e);
            t.innerHTML = '<div class="empty-state">Error loading scripts</div>';
        }
    })
}
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 3. Cargar scripts coincidentes para la pestaña actual
    loadMatchedScripts();

    // 4. Verificar si hay userscripts detectables (si existe el elemento y hay pestaña activa)
...
document.getElementById('open-report-bug').addEventListener('click', function() {
    chrome.tabs.create({
