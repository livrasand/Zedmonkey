...
function initializeUI() {
    document.getElementById("folder-button").addEventListener("click", toggleAllScriptsView), document.getElementById("cloud-button").addEventListener("click", toggleUpdatesView), document.getElementById("refresh-button").addEventListener("click", refreshPopup), document.getElementById("injection-toggle").addEventListener("change", toggleInjection), document.getElementById("open-extension-page").addEventListener("click", openExtensionPage), document.getElementById("add-script").addEventListener("click", showAddScriptForm), document.getElementById("import-script").addEventListener("click", importScript), chrome.storage.sync.get("injectionEnabled", e => {
        e = !1 !== e.injectionEnabled;
        document.getElementById("injection-toggle").checked = e
    }), checkRestrictedSite(), checkForNoScripts(), checkForUpdates()
}
...
function toggleInjection(e) {
    ...
    chrome.storage.sync.set({
        ...
        t && t[0] && (r ? chrome.runtime.sendMessage({
            action: "getMatchedScripts",
            url: t[0].url
        }, e => {
            e = (e?.scripts || []).filter(e => !1 !== e.enabled);
...
function hideAddScriptForm() {
    document.getElementById("add-script-form").classList.add("hidden"), document.getElementById("script-content").value = "", document.getElementById("script-content").dataset.scriptId = ""
}

function saveScript() {
    var e, t = document.getElementById("script-content").value.trim();
    let r = document.getElementById("script-content").dataset.scriptId;
    t ? (e = r ? "updateScriptContent" : "addScript", chrome.runtime.sendMessage({
        action: e,
        scriptContent: t,
        scriptId: r
    }, e => {
        e.success ? (hideAddScriptForm(), refreshPopup()) : alert(`Error ${r?"updating":"adding"} script: ` + e.error)
...
function installDetectedScript() {
    var e, t = document.getElementById("userscript-alert");
    t ? (e = t.dataset.scriptContent) ? (console.log("Storing script content for editor:", e.substring(0, 100) + "..."), chrome.storage.local.set({
        tempScriptContent: e
    }, function() {
        chrome.runtime.lastError ? console.error("Error storing script content:", chrome.runtime.lastError) : chrome.tabs.create({
            url: chrome.runtime.getURL("editor/editor.html?loadTemp=true")
        })
    }), t.classList.add("hidden")) : console.error("No script content found in userscript-alert element") : console.error("Userscript alert element not found in DOM")
}
...
function refreshPopup() {
    var e = document.getElementById("updates-view"),
        t = document.getElementById("all-scripts-view");
    (e.classList.contains("hidden") ? t.classList.contains("hidden") ? loadMatchedScripts : loadAllScripts : loadUpdates)(), chrome.tabs.query({
        active: !0,
        currentWindow: !0
    }, e => {
        e[0] && checkForUserscripts(e[0].url)
    }), checkRestrictedSite(), checkForUpdates()
}
...
function checkForUserscripts(e) {
    chrome.tabs.query({
        ...
        e && e[0] ? chrome.tabs.sendMessage(e[0].id, {
            ...
            var t, r, n;
            chrome.runtime.lastError ? console.log("Content script not ready:", chrome.runtime.lastError.message) : e && e.detected && ((t = document.getElementById("userscript-alert")).classList.remove("hidden"), r = extractScriptName(e.scriptContent), (n = document.getElementById("detected-script-name")) && (n.textContent = r || "Unknown Script"), t.dataset.scriptContent = e.scriptContent)
        }) : console.error("No active tab found")
    })
}
    ...
    chrome.runtime.sendMessage({
        ...
        0 === e.length ? o.innerHTML = '<div class="empty-state">No updates available</div>' : (o.innerHTML = "", e.forEach(e => {
            var t = document.createElement("div"),
                ...
                s = (c.className = "script-version", c.textContent = `v${e.currentVersion} → v` + e.newVersion, document.createElement("div")),
                i = (s.className = "script-actions", document.createElement("button"));
            i.className = "btn-icon update", i.innerHTML = "↑", i.title = "Update script", i.addEventListener("click", () => updateScript(e.id)), r.appendChild(n), r.appendChild(c), s.appendChild(i), t.appendChild(r), t.appendChild(s), o.appendChild(t)
        }))
    })
}
...
document.addEventListener("DOMContentLoaded", async () => {
    var e = (await chrome.tabs.query({
        ...
        t = (initializeUI(), document.getElementById("userscript-alert"));
    t && e && checkForUserscripts(e.url), loadMatchedScripts()
});