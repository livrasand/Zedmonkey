...
let urlParams = new URLSearchParams(window.location.search),
    ...
    scriptName = urlParams.get("name") || "Hello World Script",
    domElements = {};
...
function initializeEditor() {
    ...
    scriptContent.addEventListener("input", () => {
        ...
        // Actualizar el título del script en tiempo real
        updateScriptTitle();
...
// Añadir la función para actualizar el título del script en tiempo real
function updateScriptTitle() {
    const content = document.getElementById("script-content").value;
    const titleElement = document.getElementById("script-title");
    
    // Buscar el nombre del script en el formato Zedata Block
    const zedataMatch = content.match(/\/\/\s*name:([^,]+),/);
    
    // Buscar el nombre del script en el formato UserScript tradicional
    const userscriptMatch = content.match(/@name\s+(.+?)(\n|$)/);
    
    if (zedataMatch && zedataMatch[1]) {
        titleElement.textContent = zedataMatch[1].trim();
    } else if (userscriptMatch && userscriptMatch[1]) {
        titleElement.textContent = userscriptMatch[1].trim();
    }
}
