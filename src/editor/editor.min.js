let jsGrammar=[];function loadGrammarRules(){console.log("Using fallback grammar due to CSP restrictions"),setupFallbackGrammar()}function highlightMatchingBrackets(s,a){if(s&&void 0!==a){var e={"(":")","{":"}","[":"]"},i={")":"(","}":"{","]":"["},c=s[a],l=0<a?s[a-1]:null;let n=-1,r,o;if(l&&e[l]){r=l,o=e[l];let t=1;for(let e=a;e<s.length;e++)if(s[e]===r)t++;else if(s[e]===o&&0===--t){n=e;break}}else if(c&&i[c]){o=c,r=i[c];let t=1;for(let e=a-1;0<=e;e--)if(s[e]===o)t++;else if(s[e]===r&&0===--t){n=e;break}}-1!==n&&console.log("Matching brackets found at positions:",a,"and",n)}}function setupFallbackGrammar(){jsGrammar=[{name:"comment",pattern:new RegExp("//.*|/\\*[\\s\\S]*?\\*/","y")},{name:"string",pattern:new RegExp("\"(?:\\\\.|[^\"\\\\])*\"|'(?:\\\\.|[^'\\\\])*'|`(?:\\\\.|[^`\\\\])*`","y")},{name:"number",pattern:new RegExp("\\b\\d+(\\.\\d+)?\\b","y")},{name:"keyword",pattern:new RegExp("\\b(?:function|const|let|var|if|else|for|while|return)\\b","y")},{name:"boolean",pattern:new RegExp("\\b(?:true|false)\\b","y")},{name:"operator",pattern:new RegExp("[=+\\-*/%<>!&|~^?:]+","y")},{name:"punctuation",pattern:new RegExp("[{}\\[\\]();,.]","y")},{name:"identifier",pattern:new RegExp("[a-zA-Z_$][a-zA-Z0-9_$]*","y")}],document.getElementById("script-content")&&highlightSyntax()}loadGrammarRules();let urlParams=new URLSearchParams(window.location.search),scriptId=urlParams.get("id"),scriptName=urlParams.get("name")||"New Script",domElements={};function getElement(e){return domElements[e]||(domElements[e]=document.getElementById(e)),domElements[e]}function deleteScript(){scriptId?confirm("Are you sure you want to delete this script?")&&chrome.runtime.sendMessage({action:"removeScript",scriptId:scriptId},e=>{e&&e.success?(showStatusMessage("Script deleted"),setTimeout(()=>{window.location.href="editor.html"},1e3)):showStatusMessage("Error deleting script",!0)}):showStatusMessage("Cannot delete new script",!0)}function tokenizeLine(t){var n=[];let r=0;if(t.includes("// ==UserScript==")||t.includes("// ==/UserScript=="))n.push({type:"meta-section",text:t});else{if(t.match(/\/\/ @\w+/)){var e=t.match(/^(\/\/ @\w+)(\s+)(.+?)$/);if(e)return n.push({type:"meta",text:e[1]}),n.push({type:"plain",text:e[2]}),n.push({type:"string",text:e[3]}),n}for(;r<t.length;){let e=!1;for(var o of jsGrammar){o.pattern.lastIndex=r;var s=o.pattern.exec(t);if(s&&s.index===r){n.push({type:o.name,text:s[0]}),r+=s[0].length,e=!0;break}}e||(n.push({type:"plain",text:t[r]}),r++)}}return n}function renderLineTokens(e){return e.map(e=>`<span class="token ${e.type}">${e.text}</span>`).join("")}function highlightSyntax(){let t=document.getElementById("script-content"),n=document.getElementById("highlight-layer");t&&n&&requestAnimationFrame(()=>{var e=applyHighlightingAdvanced(t.value);n.innerHTML=e,n.scrollTop=t.scrollTop,n.scrollLeft=t.scrollLeft,highlightMatchingBrackets(t.value,t.selectionStart)})}function applyHighlightingAdvanced(e){return e?e.replace(/(\/\/.*$)/gm,'<span class="token comment">$1</span>').replace(/(\/\*[\s\S]*?\*\/)/g,'<span class="token comment">$1</span>').replace(/("(?:\\.|[^"\\])*")|('(?:\\.|[^'\\])*')|(`(?:\\.|[^`\\])*`)/g,'<span class="token string">$&</span>').replace(/\b(-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)\b/g,'<span class="token number">$1</span>').replace(/\b(function|const|let|var|return|if|else|for|while|do|switch|case|break|continue|new|try|catch|finally|throw|class|extends|import|export|default|from|async|await|this|super|yield|typeof|instanceof|in|of|delete|void|null|undefined|true|false)\b/g,'<span class="token keyword">$1</span>').replace(/(\b\w+\b)(?=\s*\()/g,'<span class="token function">$1</span>').replace(/(\w+)\.(\w+)/g,'<span class="token object">$1</span><span class="token punctuation">.</span><span class="token property">$2</span>').replace(/(function\s+\w+)\s*\(([^)]*)\)/g,(e,t,n)=>`${t}<span class="token punctuation">(</span>${n.replace(/(\w+)/g,'<span class="token parameter">$1</span>')}<span class="token punctuation">)</span>`).replace(/(\+|\-|\*|\/|%|=|==|===|!=|!==|>|<|>=|<=|&&|\|\||!|\?|:|&|\||\^|~|<<|>>|>>>|\+=|-=|\*=|\/=|%=)/g,'<span class="token operator">$1</span>').replace(/([{}[\]()<>.,;:])/g,'<span class="token punctuation">$1</span>').replace(/\b([A-Z_][A-Z0-9_]+)\b/g,'<span class="token constant">$1</span>'):""}function updateCursorPosition(){let e=document.getElementById("script-content"),t=document.getElementById("cursor-position"),n=e.value,r=e.selectionStart,o=1,s=1;for(let e=0;e<r;e++)"\n"===n[e]?(o++,s=1):s++;t.textContent=`Line: ${o}, Column: `+s,highlightCurrentLine(o)}function highlightCurrentLine(e){var t=document.getElementById("highlight-layer"),n=t.innerHTML.split("\n");for(let e=0;e<n.length;e++)n[e]=n[e].replace(/<div class="current-line">(.*?)<\/div>/g,"$1");0<e&&e<=n.length&&(n[e-1]=`<div class="current-line">${n[e-1]}</div>`),t.innerHTML=n.join("\n")}function loadTempContentIfNeeded(){"true"===new URLSearchParams(window.location.search).get("loadTemp")&&chrome.storage.local.get("tempScriptContent",e=>{var t;e.tempScriptContent?((t=document.getElementById("script-content"))&&(t.value=e.tempScriptContent,updateLineNumbers(),highlightSyntax()),chrome.storage.local.remove("tempScriptContent")):console.error("No temporary script content found")})}let highlightTimeout;function initializeEditor(){let t=document.getElementById("script-content"),n=document.getElementById("highlight-layer");if(n){document.getElementById("save-script").addEventListener("click",saveScript),document.getElementById("cancel-edit").addEventListener("click",cancelEdit);let e=document.getElementById("download-script");e&&e.addEventListener("click",downloadScript),(e=document.getElementById("refresh-script"))&&e.addEventListener("click",refreshScript),(e=document.getElementById("delete-script"))&&e.addEventListener("click",deleteScript),(e=document.getElementById("log-button"))&&e.addEventListener("click",logScriptContent),t.addEventListener("input",()=>{clearTimeout(highlightTimeout),updateLineNumbers(),updateCursorPosition(),t.value.length<1e4?highlightSyntax():highlightTimeout=setTimeout(highlightSyntax,200)}),t.addEventListener("scroll",()=>{syncScroll(),n.scrollTop=t.scrollTop,n.scrollLeft=t.scrollLeft}),t.addEventListener("keydown",handleTabKey),t.addEventListener("click",updateCursorPosition),t.addEventListener("keyup",updateCursorPosition),document.getElementById("search-scripts").addEventListener("input",filterScripts),loadAllScripts(),scriptId?loadScriptContent(scriptId):(t.value=getScriptTemplate(),updateLineNumbers(),highlightSyntax()),urlParams.get("loadTemp")&&chrome.storage.local.get("tempScriptContent",function(e){e.tempScriptContent&&(t.value=e.tempScriptContent,updateLineNumbers(),highlightSyntax(),chrome.storage.local.remove("tempScriptContent"))})}else console.error("Highlight layer not found!")}function getScriptTemplate(){return`// [script]
...
`}function loadAllScripts(){console.log("loadAllScripts: Requesting scripts from background."),chrome.runtime?chrome.runtime.sendMessage({action:"getScripts"},e=>{chrome.runtime.lastError?(console.error("Runtime error:",chrome.runtime.lastError.message),showStatusMessage("Connection error: "+chrome.runtime.lastError.message,!0)):e?e.scripts?(console.log("loadAllScripts: Received scripts from background:",e.scripts),renderScriptsList(e.scripts)):e.error?(console.error("loadAllScripts: Error received from background:",e.error),showStatusMessage("Error loading scripts: "+e.error,!0)):(console.error("loadAllScripts: Unexpected response format:",e),showStatusMessage("Unexpected response format",!0)):(console.error("loadAllScripts: No response from background script"),showStatusMessage("No response from background script",!0),(e=document.getElementById("scripts-list"))&&(e.innerHTML='<div class="empty-state">No scripts installed</div>'))}):(console.error("Chrome runtime not available"),showStatusMessage("Chrome runtime not available",!0))}function renderScriptsList(e){console.log("renderScriptsList: Attempting to render scripts:",e);let a=document.getElementById("scripts-list");a.innerHTML="",0===e.length?a.innerHTML='<div class="empty-state">No scripts installed</div>':e.forEach(e=>{var t=document.createElement("div"),n=(t.className="script-item",e.id===scriptId&&t.classList.add("active"),getScriptType(e)),r=(t.style.borderLeftColor="css"===n?"#4caf50":"bookmarklet"===n?"#2196f3":"#ffcc00",document.createElement("div")),o=(r.className="script-info",document.createElement("div")),s=(o.style.display="flex",o.style.alignItems="center",document.createElement("span"));s.className="script-badge "+n,s.textContent=n.toUpperCase(),o.appendChild(s),(n=document.createElement("div")).className="script-name",n.style.display="inline",n.textContent=e.metadata?.name||"Unnamed Script",o.appendChild(n),r.appendChild(o),(s=document.createElement("div")).className="script-meta",s.textContent=`v${e.metadata?.version||"1.0"} • `+(e.metadata?.match?.[0]||"*"),r.appendChild(s),e.metadata?.description&&((n=document.createElement("div")).className="script-description",n.textContent=e.metadata.description,r.appendChild(n)),(o=document.createElement("label")).className="toggle-switch",(s=document.createElement("input")).type="checkbox",s.checked=!1!==e.enabled,s.dataset.scriptId=e.id,s.addEventListener("change",toggleScriptEnabled),(n=document.createElement("span")).className="slider",o.appendChild(s),o.appendChild(n),t.appendChild(r),t.appendChild(o),r.addEventListener("click",()=>{window.location.href=`editor.html?id=${e.id}&name=`+encodeURIComponent(e.metadata?.name||"Script")}),a.appendChild(t)})}function getScriptType(e){if(e.content){if(e.metadata?.type)return e.metadata.type.toLowerCase();if(!e.content.includes("@resource")&&!e.content.includes("@require")){if(e.content.trim().startsWith("javascript:"))return"bookmarklet";e.content.includes("@-moz-document")||e.content.includes("@namespace")||e.content.includes("@media")}}return"js"}function toggleScriptEnabled(t){let e=t.target.dataset.scriptId,n=t.target.checked;chrome.runtime.sendMessage({action:"toggleScriptEnabled",scriptId:e,enabled:n},e=>{e&&e.success?showStatusMessage("Script "+(n?"enabled":"disabled")):(showStatusMessage("Error updating script state",!0),t.target.checked=!n)})}function filterScripts(e){let r=e.target.value.toLowerCase();document.querySelectorAll(".script-item").forEach(e=>{var t=e.querySelector(".script-name").textContent.toLowerCase(),n=e.querySelector(".script-meta").textContent.toLowerCase();t.includes(r)||n.includes(r)?e.style.display="flex":e.style.display="none"})}function loadScriptContent(e){chrome.runtime.sendMessage({action:"getScriptContent",scriptId:e},e=>{e&&e.content?(document.getElementById("script-content").value=e.content,updateLineNumbers(),showStatusMessage("Script loaded successfully")):showStatusMessage("Error loading script",!0)})}function saveScript(){var e,t=document.getElementById("script-content").value.trim();t?(e=scriptId?"updateScriptContent":"addScript",chrome.runtime.sendMessage({action:e,scriptContent:t,scriptId:scriptId},e=>{e&&e.success?(showStatusMessage("Script saved successfully"),!scriptId&&e.scriptId&&(window.location.href=`editor.html?id=${e.scriptId}&name=`+encodeURIComponent(e.name||"Script"))):showStatusMessage("Error saving script: "+(e.error||"Unknown error"),!0)})):showStatusMessage("Please enter script content",!0)}function cancelEdit(){window.close()}function logScriptContent(){console.log(document.getElementById("script-content").value),showStatusMessage("Script content logged to console")}function updateLineNumbers(){var e=document.getElementById("script-content"),t=document.getElementById("line-numbers"),n=e.value.split("\n").length;t.innerHTML="";for(let e=1;e<=Math.max(10,n);e++){var r=document.createElement("div");r.textContent=e,t.appendChild(r)}}function syncScroll(){var e=document.getElementById("script-content");document.getElementById("line-numbers").scrollTop=e.scrollTop}function handleTabKey(e){var t,n;"Tab"===e.key&&(e.preventDefault(),t=(e=e.target).selectionStart,n=e.selectionEnd,e.value=e.value.substring(0,t)+"  "+e.value.substring(n),e.selectionStart=e.selectionEnd=t+2)}function showStatusMessage(e,t=!1){let n=document.getElementById("status-message");n.textContent=e,n.style.color=t?"#f44336":"#4caf50",setTimeout(()=>{n.textContent=""},3e3)}function downloadScript(){var e=document.getElementById("script-content").value,t=document.getElementById("script-title").textContent.replace(/[^a-z0-9]/gi,"_").toLowerCase()+".js",e=new Blob([e],{type:"text/javascript"});let n=URL.createObjectURL(e),r=document.createElement("a");r.href=n,r.download=t,document.body.appendChild(r),r.click(),setTimeout(()=>{document.body.removeChild(r),URL.revokeObjectURL(n)},0),showStatusMessage("Script downloaded")}function refreshScript(){scriptId?(loadScriptContent(scriptId),showStatusMessage("Script refreshed")):showStatusMessage("Cannot refresh new script",!0)}function addModalStyles(){var e=document.createElement("style");e.textContent=`
    ...
    `,document.head.appendChild(e)}function initializeImport(){var t=document.getElementById("import-button");let n=document.getElementById("import-dropdown");if(t&&n){var r=document.getElementById("import-local"),o=document.getElementById("import-url"),s=document.getElementById("import-cloud"),a=document.getElementById("create-new-script");addModalStyles();let e=document.createElement("input");e.type="file",e.id="file-input",e.accept=".js,.user.js,.txt",e.style.display="none",document.body.appendChild(e),createUrlImportModal(),t.addEventListener("click",e=>{e.stopPropagation(),n.classList.toggle("show")}),document.addEventListener("click",()=>{n.classList.remove("show")}),n.addEventListener("click",e=>{e.stopPropagation()}),a&&a.addEventListener("click",()=>{scriptId?window.location.href="editor.html":(document.getElementById("script-content").value=getScriptTemplate(),updateLineNumbers(),highlightSyntax(),showStatusMessage("New script template loaded")),n.classList.remove("show")}),r&&r.addEventListener("click",()=>{e.click(),n.classList.remove("show")}),e.addEventListener("change",handleFileImport),o&&o.addEventListener("click",()=>{showUrlImportModal(),n.classList.remove("show")}),s&&s.addEventListener("click",()=>{handleCloudImport(),n.classList.remove("show")})}}function createUrlImportModal(){let t=document.createElement("div");t.className="modal-overlay",t.id="url-modal",t.innerHTML=`
