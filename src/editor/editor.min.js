...
function loadGrammarRules() {
    // Skip loading external grammar due to CSP restrictions
    // Use the built-in fallback grammar instead
    console.log("Using fallback grammar due to CSP restrictions");
...
function highlightSyntax() {
    ...
    requestAnimationFrame(() => {
        ...
        const highlighted = applyHighlightingAdvanced(editor.value);
...
function applyHighlightingAdvanced(e) {
    return e ? e
        // Comentarios
...
function loadTempContentIfNeeded() {
    ...
    if ("true" === urlParams.get("loadTemp")) {
        console.log("Loading temporary content...");
        chrome.storage.local.get("tempScriptContent", e => {
            console.log("Storage result:", e);
            if (e.tempScriptContent) {
                const scriptContent = document.getElementById("script-content");
                if (scriptContent) {
                    console.log("Setting script content:", e.tempScriptContent.substring(0, 100) + "...");
                    scriptContent.value = e.tempScriptContent;
                    updateLineNumbers();
                    highlightSyntax();
                    showStatusMessage("Script imported successfully!", false);
                } else {
                    console.error("Script content element not found");
                    showStatusMessage("Editor not ready. Please try again.", true);
                }
                // Clear the temporary content
                chrome.storage.local.remove("tempScriptContent");
            } else {
                console.error("No temporary script content found in storage");
                showStatusMessage("No temporary script content found. The content may have been cleared or not saved properly.", true);
            }
        });
    }
}
...
function loadAllScripts() {
    console.log("loadAllScripts: Requesting scripts from background.");
    
    // Check if chrome runtime is available
    if (!chrome.runtime) {
        console.error("Chrome runtime not available");
        showStatusMessage("Chrome runtime not available", true);
        return;
    }
    ...
    chrome.runtime.sendMessage({
        ...
        if (chrome.runtime.lastError) {
            ...
            showStatusMessage("Connection error: " + chrome.runtime.lastError.message, true);
            return;
        }
        
        // Check if we got a valid response
        if (!response) {
            console.error("loadAllScripts: No response from background script");
            showStatusMessage("No response from background script", true);
            // Show empty state
            let scriptsListElement = document.getElementById("scripts-list");
        ...
        if (response.scripts) {
            console.log("loadAllScripts: Received scripts from background:", response.scripts);
            renderScriptsList(response.scripts);
        } else if (response.error) {
            console.error("loadAllScripts: Error received from background:", response.error);
            showStatusMessage("Error loading scripts: " + response.error, true);
        } else {
            console.error("loadAllScripts: Unexpected response format:", response);
            showStatusMessage("Unexpected response format", true);
        }
    });
}

function renderScriptsList(e) {
    console.log("renderScriptsList: Attempting to render scripts:", e);
