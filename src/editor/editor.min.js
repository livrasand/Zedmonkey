...
`}function loadAllScripts(){chrome.runtime.sendMessage({action:"getScripts"},e=>{e&&e.scripts?renderScriptsList(e.scripts):showStatusMessage("Error loading scripts",!0)})}function renderScriptsList(e){let a=document.getElementById("scripts-list");a.innerHTML="",0===e.length?a.innerHTML='<div class="empty-state">No scripts installed</div>':e.forEach(e=>{var t=document.createElement("div"),n=(t.className="script-item",e.id===scriptId&&t.classList.add("active"),getScriptType(e)),s=(t.style.borderLeftColor="css"===n?"#4caf50":"bookmarklet"===n?"#2196f3":"#ffcc00",document.createElement("div")),o=(s.className="script-info",document.createElement("div")),i=(o.style.display="flex",o.style.alignItems="center",document.createElement("span"));i.className="script-badge "+n,i.textContent=n.toUpperCase(),o.appendChild(i),(n=document.createElement("div")).className="script-name",n.style.display="inline",n.textContent=e.metadata?.name||"Unnamed Script",o.appendChild(n),s.appendChild(o),(i=document.createElement("div")).className="script-meta",i.textContent=`v${e.metadata?.version||"1.0"} • `+(e.metadata?.match?.[0]||"*"),s.appendChild(i),e.metadata?.description&&((n=document.createElement("div")).className="script-description",n.textContent=e.metadata.description,s.appendChild(n)),(o=document.createElement("label")).className="toggle-switch",(i=document.createElement("input")).type="checkbox",i.checked=!1!==e.enabled,i.dataset.scriptId=e.id,i.addEventListener("change",toggleScriptEnabled),(n=document.createElement("span")).className="slider",o.appendChild(i),o.appendChild(n),t.appendChild(s),t.appendChild(o),s.addEventListener("click",()=>{window.location.href=`editor.html?id=${e.id}&name=`+encodeURIComponent(e.metadata?.name||"Script")}),a.appendChild(t)})}function getScriptType(e){if(e.content){if(e.metadata?.type)return e.metadata.type.toLowerCase();if(!e.content.includes("@resource")&&!e.content.includes("@require")){if(e.content.trim().startsWith("javascript:"))return"bookmarklet";e.content.includes("@-moz-document")||e.content.includes("@namespace")||e.content.includes("@media")}}return"js"}function toggleScriptEnabled(t){let e=t.target.dataset.scriptId,n=t.target.checked;chrome.runtime.sendMessage({action:"toggleScriptEnabled",scriptId:e,enabled:n},e=>{e&&e.success?showStatusMessage("Script "+(n?"enabled":"disabled")):(showStatusMessage("Error updating script state",!0),t.target.checked=!n)})}function filterScripts(e){let s=e.target.value.toLowerCase();document.querySelectorAll(".script-item").forEach(e=>{var t=e.querySelector(".script-name").textContent.toLowerCase(),n=e.querySelector(".script-meta").textContent.toLowerCase();t.includes(s)||n.includes(s)?e.style.display="flex":e.style.display="none"})}function loadScriptContent(e){chrome.runtime.sendMessage({action:"getScriptContent",scriptId:e},e=>{e&&e.content?(document.getElementById("script-content").value=e.content,updateLineNumbers(),showStatusMessage("Script loaded successfully")):showStatusMessage("Error loading script",!0)})}function saveScript(){var e,t=document.getElementById("script-content").value.trim();t?(e=scriptId?"updateScriptContent":"addScript",chrome.runtime.sendMessage({action:e,scriptContent:t,scriptId:scriptId},e=>{e&&e.success?(showStatusMessage("Script saved successfully"),!scriptId&&e.scriptId&&(window.location.href=`editor.html?id=${e.scriptId}&name=`+encodeURIComponent(e.name||"Script"))):showStatusMessage("Error saving script: "+(e.error||"Unknown error"),!0)})):showStatusMessage("Please enter script content",!0)}function cancelEdit(){window.close()}function logScriptContent(){console.log(document.getElementById("script-content").value),showStatusMessage("Script content logged to console")}function updateLineNumbers(){var e=document.getElementById("script-content"),t=document.getElementById("line-numbers"),n=e.value.split("\n").length;t.innerHTML="";for(let e=1;e<=Math.max(10,n);e++){var s=document.createElement("div");s.textContent=e,t.appendChild(s)}}function syncScroll(){var e=document.getElementById("script-content");document.getElementById("line-numbers").scrollTop=e.scrollTop}function handleTabKey(e){var t,n;"Tab"===e.key&&(e.preventDefault(),t=(e=e.target).selectionStart,n=e.selectionEnd,e.value=e.value.substring(0,t)+"  "+e.value.substring(n),e.selectionStart=e.selectionEnd=t+2)}function showStatusMessage(e,t=!1){let n=document.getElementById("status-message");n.textContent=e,n.style.color=t?"#f44336":"#4caf50",setTimeout(()=>{n.textContent=""},3e3)}function downloadScript(){var e=document.getElementById("script-content").value,t=document.getElementById("script-title").textContent.replace(/[^a-z0-9]/gi,"_").toLowerCase()+".js",e=new Blob([e],{type:"text/javascript"});let n=URL.createObjectURL(e),s=document.createElement("a");s.href=n,s.download=t,document.body.appendChild(s),s.click(),setTimeout(()=>{document.body.removeChild(s),URL.revokeObjectURL(n)},0),showStatusMessage("Script downloaded")}function refreshScript(){scriptId?(loadScriptContent(scriptId),showStatusMessage("Script refreshed")):showStatusMessage("Cannot refresh new script",!0)}function deleteScript(){scriptId?confirm("Are you sure you want to permanently delete this script?")&&chrome.runtime.sendMessage({action:"removeScript",scriptId:scriptId},e=>{e?.success?(showStatusMessage("Script deleted successfully"),loadAllScripts(),setTimeout(()=>{window.location.href="editor.html"},500)):(console.error("Delete failed:",e?.error),showStatusMessage("Delete failed: "+(e?.error||"Unknown error"),!0))}):showStatusMessage("Cannot delete new script",!0)}document.getElementById("script-title").textContent=scriptName,document.addEventListener("DOMContentLoaded",initializeEditor),document.addEventListener("DOMContentLoaded",()=>{loadTempContentIfNeeded()}),document.addEventListener("DOMContentLoaded",initializeEditor),document.addEventListener("DOMContentLoaded",()=>{loadTempContentIfNeeded()}),document.addEventListener("DOMContentLoaded",initializeEditor);