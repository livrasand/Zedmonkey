...
`,"Userscript"),r="1.0",i="";e.scriptContent&&((t=e.scriptContent.match(/@name\s+(.+?)(\n|$)/))&&(o=t[1].trim()),(t=e.scriptContent.match(/@version\s+(.+?)(\n|$)/))&&(r=t[1].trim()),(t=e.scriptContent.match(/@author\s+(.+?)(\n|$)/))&&t[1].trim(),t=e.scriptContent.match(/@description\s+(.+?)(\n|$)/))&&(i=t[1].trim());var t=document.createElement("div"),c=(t.style.cssText=`
    ...
    `,document.createElement("div"));h.textContent="@match",h.style.cssText=`
        font-size: 12px;
        color: #7f8c8d;
    `;let x=[];e.scriptContent&&(f=e.scriptContent.match(/@match\s+(.+?)(\n|$)/g),g=e.scriptContent.match(/@include\s+(.+?)(\n|$)/g),f&&0<f.length&&(x=f.map(e=>e.replace(/@match\s+/,"").trim())),g)&&0<g.length&&(f=g.map(e=>e.replace(/@include\s+/,"").trim()),x=[...x,...f]);var g=document.createElement("div"),f=(g.textContent=0<x.length?x.join("\n"):"http://*/*\nhttps://*/*",g.style.cssText=`
        font-size: 12px;
        color: #ecf0f1;
    `,m.appendChild(h),m.appendChild(g),d.appendChild(c),d.appendChild(m),l.appendChild(p),l.appendChild(d),document.createElement("div"));f.style.cssText=`
        display: flex;
        gap: 5px;
    ...
    `,b.onmouseover=()=>{b.style.background="#374a5c"},b.onmouseout=()=>{b.style.background="#2c3e50"},document.createElement("button"));v.textContent="Editar",v.style.cssText=`
        ...
        font-weight: bold;
        transition: background 0.2s;
    `,v.onmouseover=()=>{v.style.background="#374a5c"},v.onmouseout=()=>{v.style.background="#2c3e50"};h=(e,t)=>{var n=e.textContent||e.innerText;e.innerHTML=`<span style="color: ${t}; margin-right: 4px;">+</span>`+n};h(b,"#95a5a6"),h(v,"#f39c12"),f.appendChild(y),f.appendChild(b),f.appendChild(v),n.appendChild(t),n.appendChild(l),n.appendChild(f),y.addEventListener("click",()=>{chrome.runtime.sendMessage({action:"addScript",scriptContent:e.scriptContent},e=>{e&&e.success?showNotification("Script instalado correctamente!"):showNotification("Error al instalar el script: "+(e?.error||"Error desconocido"),!0),n.remove()})}),v.addEventListener("click",()=>{chrome.runtime.sendMessage({action:"openScriptInEditor",scriptContent:e.scriptContent}),n.remove()}),b.addEventListener("click",()=>{n.remove()});let C=document.createElement("div");C.innerHTML="&times;",C.style.cssText=`
        position: absolute;
        top: 10px;
        right: 15px;
        font-size: 20px;
